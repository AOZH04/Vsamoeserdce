(() => {
    ...
    document.addEventListener("watcherCallback", (function(e) {
        ...
        if (entry.isIntersecting) {
            ...
            if (targetElement.classList.contains("animup")) !targetElement.classList.contains("fadeInUp") ? targetElement.classList.add("fadeInUp") : null;
        } else {
    ...
    const s = document.querySelector(".gift"), c = s.querySelector(".giftbody_center"), i = {
        tl: s.querySelector(".topleft"),
        tr: s.querySelector(".topright"),
        bl: s.querySelector(".bottomleft"),
        br: s.querySelector(".botomright")
    };
    const stickyParent = s.closest(".sticky");
    const isTouch = "ontouchstart" in window || navigator.maxTouchPoints > 0 || window.matchMedia("(pointer:coarse)").matches;
    Object.values(i).forEach((el => el && isTouch && el.classList.add("touch")));
    const step = 15;
    const max = 200;
    let script_off = 0, lastY = window.scrollY, dir = "down";
    const setDone = () => {
        if (!stickyParent) return;
        if (script_off >= max) stickyParent.classList.add("done");
    };
    const nullpos = () => Object.values(i).forEach((el => el && (0 === script_off ? el.classList.add("nullpos") : el.classList.remove("nullpos"))));
    const upd = () => {
        if (isTouch) return;
        i.tl && (i.tl.style.transform = `translate(${script_off}px,${script_off}px)`);
        i.tr && (i.tr.style.transform = `translate(${-script_off}px,${script_off}px)`);
        i.bl && (i.bl.style.transform = `translate(${script_off}px,${-script_off}px)`);
        i.br && (i.br.style.transform = `translate(${-script_off}px,${-script_off}px)`);
        nullpos();
        setDone();
    };
    nullpos();
    if (!isTouch) {
        ...
        const onWheel = e => {
            if (!c.classList.contains("_watcher-view")) return;
            if (0 === script_off && e.deltaY < 0) return;
            e.preventDefault();
            script_off = e.deltaY > 0 ? Math.min(script_off + step, max) : Math.max(script_off - step, 0);
            upd();
            if (script_off >= max && e.deltaY > 0) {
                setDone();
        ...
        new IntersectionObserver((es => {
            es.forEach((e => {
                if (e.isIntersecting) {
                    c.classList.add("_watcher-view");
                    script_off = "down" === dir ? 0 : max;
                    upd();
                    window.addEventListener("wheel", onWheel, {
                        passive: false
                    });
                } else {
                    c.classList.remove("_watcher-view");
                    window.removeEventListener("wheel", onWheel, {
                        passive: false
                    });
                }
    ...
    if (window.innerWidth > 850) (() => {
        const smooth = el => el.scrollIntoView({
            block: "start",
            inline: "start",
            behavior: "smooth"
        });
        const watch = el => {
            const handle = m => {
                const had = m.oldValue?.split(/\s+/).includes("_watcher-view");
                const has = el.classList.contains("_watcher-view");
                if (!had && has) {
                    el.classList.add("centered");
                    smooth(el);
                }
            };
            new MutationObserver((ms => ms.forEach((m => "attributes" === m.type && "class" === m.attributeName && handle(m))))).observe(el, {
                attributes: true,
                attributeFilter: [ "class" ],
                attributeOldValue: true
            });
            if (el.classList.contains("_watcher-view")) {
                el.classList.add("centered");
                smooth(el);
            }
        };
    ...
    if (window.innerWidth > 850) (() => {
        ...
        const onceReady = cb => {
            const el = document.querySelector(".gift");
            if (el) return cb(el);
            new MutationObserver(((_, o) => {
                const e = document.querySelector(".gift");
                if (e) {
                    o.disconnect();
                    cb(e);
                }
    ...
    document.querySelectorAll(".custom_audio").forEach((wrap => {
        const audio = wrap.querySelector("audio");
        const btnPlay = wrap.querySelector(".playbtn");
        const btnPause = wrap.querySelector(".pausebtn");
        const trackBox = wrap.querySelector(".audioheartbox");
        const layerBox = wrap.querySelector(".audioheartlayerbox");
        let durationSec = 0;
        let tick = null;
        const setDuration = () => {
            durationSec = Math.max(1, Math.floor(audio.duration || 0));
        };
        const updateProgress = () => {
            const w = trackBox.clientWidth || 0;
            const t = Math.floor(audio.currentTime || 0);
            const p = Math.min(t / durationSec, 1);
            layerBox.style.width = w * p + "px";
            layerBox.style.opacity = p > 0 ? "1" : "0";
        };
        const startTicks = () => {
            clearInterval(tick);
            tick = setInterval(updateProgress, 1e3);
        };
        const stopTicks = () => clearInterval(tick);
    ...
    document.querySelectorAll(".custom_audio").forEach((wrap => {
        ...
        const trackBox = wrap.querySelector(".audioheartbox");
        const layerBox = wrap.querySelector(".audioheartlayerbox");
        const timeEl = wrap.querySelector(".audiotime");
        let durationSec = 0;
        let tick = null;
        const fmt = s => {
        ...
        const setDuration = () => {
            ...
            durationSec = Math.max(1, Math.floor(audio.duration || 0));
            if (timeEl) timeEl.textContent = fmt(durationSec);
        };
        const updateProgressAndTime = () => {
            ...
            layerBox.style.width = w * p + "px";
            layerBox.style.opacity = p > 0 ? "1" : "0";
            if (timeEl) timeEl.textContent = fmt(durationSec - audio.currentTime);
        };
        const startTicks = () => {
            clearInterval(tick);
            tick = setInterval(updateProgressAndTime, 1e3);
        };
        const stopTicks = () => {
            clearInterval(tick);
        };
        ...
        audio.addEventListener("seeked", updateProgressAndTime);
        ...
        window.addEventListener("resize", updateProgressAndTime);
    }));
    window["FLS"] = false;
    menuInit();
    functions_menuClose();
